...
const Navbar = ({ isEditingMode = false }) => {
  ...
  const navigation = [
    ...
    { name: 'Upload', href: '/upload', icon: '⬆️', description: 'Upload CSV files' },
    { name: 'Projects', href: '/projects', icon: '📁', description: 'Manage projects' },
  ]

  const designTools = [
    { name: 'Templates', icon: '🎨', action: () => {}, description: 'Browse templates' },
    { name: 'Themes', icon: '🌈', action: () => {}, description: 'Islamic themes' },
    { name: 'Fonts', icon: '📝', action: () => {}, description: 'Typography' },
    { name: 'Export', icon: '💾', action: () => {}, description: 'Export posts' },
  ]

  return (
    <nav className={`bg-white shadow-lg border-b border-gray-200 relative z-50 transition-all duration-300 ${isEditor ? 'h-12' : 'h-16'}`}>
      <div className="max-w-7xl mx-auto px-4">
        <div className={`flex justify-between items-center ${isEditor ? 'h-12' : 'h-16'}`}>
          {/* Logo & Brand */}
          <Link to="/" className="flex items-center space-x-3 group">
            <div className={`${isEditor ? 'w-8 h-8' : 'w-10 h-10'} bg-gradient-to-br from-blue-600 to-purple-600 rounded-xl flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all duration-200`}>
              <span className={`text-white font-bold ${isEditor ? 'text-sm' : 'text-lg'}`}>IT</span>
            </div>
            {!isEditor && (
            ...
            {isEditor && (
              <span className="text-sm font-medium text-gray-900">Post Editor</span>
            )}
          </Link>
            ...
            <div className="flex items-center space-x-3">
              {!isEditor && (
                <div className="hidden md:block text-right">
                  <p className="text-sm font-medium text-gray-900">Creator Mode</p>
                  <p className="text-xs text-gray-500">Ready to design</p>
                </div>
              )}
              
              <div className={`${isEditor ? 'w-8 h-8' : 'w-10 h-10'} bg-gradient-to-br from-green-500 to-blue-500 rounded-full flex items-center justify-center text-white font-semibold shadow-lg hover:shadow-xl transition-all duration-200 cursor-pointer`}>
      ...
      {(location.pathname === '/upload' || isEditor) && (
        <div className="h-1 bg-gray-200">
          <div className="h-full bg-gradient-to-r from-blue-500 to-purple-500 transition-all duration-300" 
               style={{ width: isEditor ? '75%' : '50%' }}></div>
        </div>
      )}
    </nav>
  )
